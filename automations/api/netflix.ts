...
import {
  NetflixCancelResponse,
  NetflixResumeResponse,
  ReactContext,
} from "./utils/netflix.types";
...
>(
  ...
  return getReactContext(client, item.user)
    ...
    .andThen((apiData) => {
      ...
      return doCancelResumeRequest<T>(
        client,
        paramToUse,
        apiData.authUrl,
        item.user
...
export const cancel = api(({ item, client }) => {
  return cancelOrResume<NetflixCancelResponse>("cancel", client, item).map(
    (json) => ({
      data: {
...
export const resume = api(({ item, client }) => {
  return cancelOrResume<NetflixResumeResponse>("resume", client, item).map(
    (json) => ({
      data: {
        billingCycle: "monthly",
        membershipStatus: "active",
        membershipPlan: json.currentPlan.fields.localizedPlanName.value,
        nextPaymentDate: extractDate(
          json.periodEndDate?.value ?? json.nextBillingDate?.value ?? ""
        ),
        nextPaymentPrice: extractAmount(
          json.currentPlan.fields.planPrice.value
        ),
        lastSyncedAt: new Date().toISOString(),
      },
    })
...
const PAGE_ITEM_ERROR_CODE = (json: any) =>
  json?.models?.flow?.data?.fields?.errorCode?.value;
  ...
  return client
    ...
    .andThen(({ cookies, data }) => {
      if (!data.value.moneyball.appleSignUp.login.accountOwnerGuid) {
        console.log(
          "Error",
          data.value.moneyball.appleSignUp.login.fields?.errorCode?.value
        );
        return Err({
    ...
    .andThen(({ data, cookies }) => {
      ...
      const periodEndDate =
        data?.models?.signupContext?.data?.flow?.fields?.periodEndDate?.value;
      const nextBillingDate =
        data?.models?.signupContext?.data?.flow?.fields?.nextBillingDate?.value;
      const planName =
        data?.models?.signupContext?.data?.flow?.fields?.currentPlan?.fields
          ?.localizedPlanName?.value;
      const planPrice =
        data?.models?.signupContext?.data?.flow?.fields?.currentPlan?.fields
          ?.planPrice?.value;
