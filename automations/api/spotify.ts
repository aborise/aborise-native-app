...
import { Err, fromPromise } from "~/shared/Result";
  ...
  return client
    ...
    .map(({ document, cookies, response }) => {
      const data = JSON.parse(
        document
          .getElementById("bootstrap-data")
          ?.getAttribute("sp-bootstrap-data") ?? "null"
      );
      const src = document
        .querySelector("script[sp-bootstrap]")
        ?.getAttribute("src");
    ...
    .andThen(({ siteKey, cookies, flowCtx }) => {
      const csrf = cookies.find(
        (cookie) => cookie.name === "sp_sso_csrf_token"
      )?.value;
